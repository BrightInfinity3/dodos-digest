...
// Small pools (e.g. anonymous TheCatAPI returns 10/request) sit below MIN_ITEMS
// forever — without a cooldown every feed request would hit that upstream again.
const MIN_REFILL_INTERVAL = 60 * 1000;

// Called on every feed request: kick off refills where needed, don't wait.
function ensureFresh() {
  for (const name of Object.keys(pools)) {
    const pool = pools[name];
    const sinceRefill = Date.now() - pool.lastRefill;
    const stale = sinceRefill > TTL[name];
    const low = pool.items.length < MIN_ITEMS && sinceRefill > MIN_REFILL_INTERVAL;
    if (pool.items.length === 0 || low || stale) {
