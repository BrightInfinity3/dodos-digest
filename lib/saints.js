...
// A "Taken on …" date means it's a modern photograph OF the artwork, not the
// artwork's own date, so we drop it rather than mislabel a centuries-old work.
function cleanDate(raw) {
  let d = stripHtml(raw);
  if (/^\s*(taken on|photographed|scanned|uploaded)/i.test(d)) return "";
  d = d.replace(/date\s*QS:.*$/i, "").replace(/QS:.*$/i, "")
    .replace(/\+?\d{4}-\d{2}-\d{2}T[\d:.Z/-]*/g, "").trim();
  if (!d || d.length > 40 || /[{}]|http/i.test(d)) return "";
  return d;
}

// Distinctive subject patterns (accent-normalized) so an image counts as art OF
// the saint only when the SAINT — not a same-named painter or a ship/building —
// is the subject. Matched against the file title and against art-category names.
const SUBJECT = {
  "francis-of-assisi": /francis of assisi/,
  "therese-of-lisieux": /therese (of|de) lisieux|therese martin/,
  "anthony-of-padua": /anthony of padua|antonius van padua|antonio da padova|antoine de padoue|antonio de padua/,
  "joseph": /saint ?joseph|st\.? ?joseph|san ?jose|s\.? ?giuseppe|heiliger josef/,
  "patrick": /saint ?patrick|st\.? ?patrick|s\.? ?patrizio|san patricio/,
  "nicholas": /nicholas of myra|saint ?nicholas|st\.? ?nicholas|san nicola|heiliger nikolaus/,
  "cecilia": /saint ?cecilia|st\.? ?cecilia|santa cecilia|cecilia of rome/,
  "clare-of-assisi": /clare of assisi|santa chiara|saint ?clare|st\.? ?clare/,
  "catherine-of-siena": /catherine of siena|caterina da siena|santa caterina|saint ?catherine of siena/,
  "teresa-of-avila": /teresa of avila|teresa de avila|teresa de jesus|santa teresa|saint ?teresa/,
  "michael-the-archangel": /archangel michael|michael the archangel|saint ?michael|st\.? ?michael|san miguel|arcangelo michele|erzengel michael|arcangel miguel/,
  "george": /saint ?george|st\.? ?george|s\.? ?giorgio|san jorge|saint georges|george.{0,8}dragon/,
};
const ART_CAT = /painting|in art|iconograph|fresco|altarpiece|\bicon|drawing|depiction|artwork|images of/i;
// Photos of buildings / statues / objects are not the 2D artworks we promise.
// Tested against the ACCENT-STRIPPED title, and building words match as
// substrings so French/Spanish/Italian forms (cathédrale, chapelle, iglesia,
// chiesa) are caught too — not just the English word-boundary versions.
const REJECT = /\b(map|coat of arms|logo|diagram|banner|flag|seal|signature|gpx|locator|geograph|statue|sculpture|plaque|medal|coin|stamp|nave|aisle|tower)\b|church|basilic|basiliq|cathedr|chapel|iglesia|eglise|chiesa|kirche|duomo|catedral|interior|exterior|panorama|facade|monaster|convent|abbey|window|stained|piazza/i;

const stripAccents = (s) => s.normalize("NFD").replace(/[̀-ͯ]/g, "").toLowerCase();
// Collapse the same artwork (language variants, "detail"/"crop" versions) to one.
function baseKey(title) {
  return stripAccents(title)
    .replace(/^[^-]{1,40}\s-\s/, "")        // drop a leading "Artist - " prefix
    .replace(/\b(detail|cropped|crop|study|sketch|fragment|after|copy|replica)\b.*$/, "")
...
async function fetchArt(saint) {
  const subject = SUBJECT[saint.slug];
  const q = encodeURIComponent(saint.query + " painting");
  const url = "https://commons.wikimedia.org/w/api.php?action=query&generator=search" +
    "&gsrsearch=" + q + "&gsrnamespace=6&gsrlimit=40&prop=imageinfo%7Ccategories" +
    "&iiprop=url%7Cextmetadata%7Cmime&iiurlwidth=1000&cllimit=max&clshow=!hidden&format=json";
  ...
  for (const p of ordered) {
    ...
    const normTitle = stripAccents(rawTitle);
    if (REJECT.test(normTitle)) continue;
    // Honesty gate: only keep it if the SAINT is the subject — proven either by
    // the file title, or by membership in an "art of this saint" category.
    const cats = (p.categories || []).map((c) => stripAccents(c.title.replace(/^Category:/, "")));
    const subjectInTitle = subject && subject.test(normTitle);
    const subjectInArtCat = subject && cats.some((c) => ART_CAT.test(c) && subject.test(c));
    if (!subjectInTitle && !subjectInArtCat) continue;
    const key = baseKey(rawTitle);
    ...
    items.push({
      ...
      title: rawTitle.replace(/\.(jpe?g|png)$/i, ""),
      artist,
      date,
